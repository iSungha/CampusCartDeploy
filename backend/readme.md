...
CampusCart is a campus buy-and-sell marketplace backend built with **Node.js**, **Express**, **MongoDB Atlas**, **Mongoose**, **JWT authentication**, **Gmail SMTP email verification**, **Cloudinary image storage**, **Gemini AI description generation**, and **Swagger API documentation**.
...
CampusCart now includes a backend-only Gemini integration:

```text
POST /api/ai/generate-description
```

The route is protected by the existing JWT middleware. The React frontend sends the listing fields to CampusCart's backend, and the backend calls Gemini. The Gemini API key is never sent to the browser.
...
The model value is optional because the code defaults to `gemini-3.5-flash-lite`.

## Request example

First log in, copy the JWT, and authorize in Swagger. Then send:
...
```json
{
  "description": "Used calculus textbook in good condition with some highlighting. It is a practical and affordable option for a student who needs a course copy without paying full retail price. Available for $35.",
  "generatedFrom": {
    "title": "Used Calculus Textbook",
    "category": "textbooks",
    "condition": "used",
...
The exact wording can differ because it is generated by Gemini. The frontend should place `description` into the normal listing-description textarea so the seller can review and edit it before publishing.
...
The backend uses Node's built-in `fetch`, so use Node.js 18 or newer. No new npm package is required.

---

## Authentication patch: logout, password reset, and Render verification

The completed patch is documented in `AUTH_PATCH_SETUP.md`.

New protected endpoints:

- `POST /api/auth/logout` invalidates all existing JWTs for the account.
- `POST /api/auth/reset-password` accepts `currentPassword` and `newPassword`,
  changes the password, and invalidates all existing JWTs.

Runtime verification behaviour:

- On localhost, registration sends the existing verification email.
- On Render, registration automatically sets `isEmailVerified` to `true`.
- `GET /api/health` reports the detected runtime and verification mode.
